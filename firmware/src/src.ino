...
boolean DEBUG = true;    // Print serial debug
...
boolean RF_STATUS = false; // Enable RF
...
void setup()
{
  ...
  if (RF_STATUS == 1)
  {
    ...
    if (digitalRead(DIP_switch1) == LOW)
      nodeID--; // IF DIP switch 1 is switched on then subtract 1 from nodeID
    Serial.print(" Node: ");
    Serial.print(nodeID);
    Serial.print(" Freq: ");
  ...
  if (analogRead(1) > 0)
  {
    ...
    CT1 = 0; // check to see if CT is connected to CT1 input, if so enable that channel
  if (analogRead(2) > 0)
  {
    CT2 = 1;
    CT_count++;
    ...
    CT2 = 0; // check to see if CT is connected to CT2 input, if so enable that channel
  if (analogRead(3) > 0)
  {
    CT3 = 1;
    CT_count++;
    ...
    CT3 = 0; // check to see if CT is connected to CT3 input, if so enable that channel
  if (analogRead(4) > 0)
  {
    CT4 = 1;
    CT_count++;
    ...
    CT4 = 0; // check to see if CT is connected to CT4 input, if so enable that channel

  if (CT_count == 0)
    CT1 = 1; // If no CT's are connect ed CT1-4 then by default read from CT1
  ...
  else
  {
    ...
    digitalWrite(LEDpin, HIGH);
    delay(2000);
    digitalWrite(LEDpin, LOW); // indicate DC power has been detected by turing LED on then off
  }
  ...
  if (DEBUG == 1)
  {
    Serial.print("CT 1 Cal ");
    Serial.println(Ical1);
    Serial.print("CT 2 Cal ");
    Serial.println(Ical2);
    Serial.print("CT 3 Cal ");
    Serial.println(Ical3);
    Serial.print("CT 4 Cal ");
    Serial.println(Ical4);
    delay(1000);
    ...
    if (ACAC)
    {
      ...
      if (USA == true)
        Serial.println("USA mode active");
      Serial.print("Vcal: ");
      Serial.println(Vcal);
    ...
    else
    {
      ...
      if (CT3)
        Serial.println("CT 3 detected");
      if (CT4)
        Serial.println("CT 4 detected");
    }
...
void loop()
{
  ...
  if (CT1 || ACAC)
  { // Awlays sample for CT's if powered by AC-AC
    if (ACAC)
    {
    ...
    { //apparent power calculation if AC-AC not connected
      emontx.power1 = ct1.calcIrms(no_of_samples) * Vrms;
    }
    if (analogRead(1) == 0)
      emontx.power1 = 0; // CT disconnected
  }
  ...
  if (CT2 || ACAC)
  {
    ...
    if (analogRead(2) == 0)
      emontx.power2 = 0; // CT disconnected
  }
  ...
  if (CT3 || ACAC)
  {
    ...
    if (analogRead(3) == 0)
      emontx.power3 = 0; // CT disconnected
  }
  ...
  if (CT4 || ACAC)
  {
    ...
    if (analogRead(4) == 0)
      emontx.power4 = 0; // CT disconnected
  }
  ...
  if (ACAC)
  { // If powered by AC-AC adaper (mains power) then delay instead of sleep
    delay(sleeptime);
  }
  else
  {                                        // if powered by battery then sleep rather than delay and disable LED to reduce energy consumption
...
void send_rf_data()
{
  ...
  rf12_sendWait(2);
  if (!ACAC)
    rf12_sleep(RF12_SLEEP); //if powered by battery then put the RF module to sleep between readings
}
